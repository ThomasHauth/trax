...
void deletePartialSums(){

	for(uint i = 0; i < partialSums.size(); ++i){
		delete partialSums[i];
	}
...
int main(int argc, char **argv) {
	...
	uint maxWG = wgScan->krnl.getWorkGroupSize();
	cl_ulong maxAlloc = contx->getMaxAllocSize();
	std::cout << "Max work group size is " << maxWG << " Max mem alloc " << maxAlloc << std::endl;
	...
	for(uint wg = 1; wg <= maxWG; wg <<= 1){
		...
		output << wg << " " << 1000.0 * t / (double)(ITERATIONS) << " " << 1e-9 * SIZE * sizeof(uint) * ITERATIONS / t << std::endl;

 ...
 */
class BoundarySelectionZ: private boost::noncopyable
{
private:
...
public:
	...
	BoundarySelectionZ(clever::context & ctext) :
		ctx(ctext),
		selectionZ_kernel(ctext)
{
		// create the buffers this algorithm will need to run
#define DEBUG_OUT
#ifdef DEBUG_OUT
		std::cout << "Boundary Selection Kernel WorkGroupSize: " << selectionZ_kernel.getWorkGroupSize() << std::endl;
#endif
#undef DEBUG_OUT
}
	...
	cl_ulong run(HitCollection & hits, uint nThreads, uint maxLayer, const LayerSupplement & layerSupplement, Grid & grid)
	{
		...
		nThreads = min(nThreads, selectionZ_kernel.getWorkGroupSize());

		std::vector<cl_event> events;
		for(uint layer = 1; layer <= maxLayer; ++layer){
			cl_event evt = selectionZ_kernel.run(
					//configuration
	...
	__kernel void selectionZ_kernel(
			//configuration

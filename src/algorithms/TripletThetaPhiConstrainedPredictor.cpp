/*
 * TripletThetaPhiConstrainedPredictor.cpp
 *
 *  Created on: May 13, 2013
 *      Author: dfunke
 */

#include "TripletThetaPhiConstrainedPredictor.h"

Pairing * TripletThetaPhiConstrainedPredictor::run(HitCollection & hits, const DetectorGeometry & geom, const GeometrySupplement & geomSupplement, const Dictionary & dict,
		...
		cl_event evt = predictCount.run(
				...
				//configuration
				layerTriplets.transfer.buffer(dThetaWindow()), layerTriplets.transfer.buffer(dPhiWindow()), layerTriplets.transfer.buffer(nCandidates()),
				// input
				...
				local_param(sizeof(cl_int), (nThreads*1)), //candidate indices
				local_param(sizeof(cl_float), (nThreads*1)), //candidate floats
				//thread config
				range(nThreads, nLayerTriplets, grid.config.nEvents),
				range(nThreads, 1,1));
		TripletThetaPhiConstrainedPredictor::events.push_back(evt);
		LOG << "done" << std::endl;
		...
		evt = prefixSum.run(m_prefixSum.get_mem(), m_prefixSum.get_count(), nThreads, TripletThetaPhiConstrainedPredictor::events);

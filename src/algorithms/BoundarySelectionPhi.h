...
#undef DEBUG_OUT
	...
	static std::string KERNEL_COMPUTE_EVT() {return "SELECTION_PHI_COMPUTE";}
	...
	cl_ulong run(HitCollection & hits, int nThreads, uint maxLayer, const LayerSupplement & layerSupplement, Grid & grid)
	{
		...
		for(uint layer = 1; layer <= maxLayer; ++layer){
			cl_event evt = selectionPhi_kernel.run(
					...
					//aux
					local_param(sizeof(cl_float), grid.config.nSectorsPhi+1),
					local_param(sizeof(cl_ushort), getNextPowerOfTwo(layerSupplement[layer-1].getNHits()*(grid.config.nSectorsPhi+1))),
					//threads
					nThreads);
